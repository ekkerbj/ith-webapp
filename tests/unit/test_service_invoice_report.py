...
from ith_webapp.reports import build_service_invoice_pdf


def test_build_service_invoice_pdf_includes_sections_and_avatax_variant(session: Session):
    ...
    pdf_bytes = build_service_invoice_pdf(session, service.service_id, region="BR", variant="avatax")

    assert pdf_bytes.startswith(b"%PDF")
    assert b"Service Invoice" in pdf_bytes
    assert b"Avatax" in pdf_bytes
    assert b"Fab Number Lines" in pdf_bytes
...
def test_service_invoice_report_route_returns_pdf(app):
    ...
    try:
        ...
        response = app.test_client().get(
            f"/reports/service-invoice/{service.service_id}?region=MX&variant=avatax"
        )
