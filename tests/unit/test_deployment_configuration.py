...
def test_start_script_honors_database_url_override():
    ...
    assert 'DATABASE_URL="${DATABASE_URL:-sqlite:///ith_import.db}"' in start_script


def test_start_script_defaults_to_imported_database():
    root = Path(__file__).resolve().parents[2]
    start_script = (root / "start.sh").read_text()
...
def test_start_script_does_not_run_alembic_on_local_launch():
    root = Path(__file__).resolve().parents[2]
    start_script = (root / "start.sh").read_text()
